    ...
    class ban : IChatCommand
    {
        public void Execute(GameClient Session, Room Room, RoomUser UserRoom, string[] Params)
        {
            ...
            else
            {
                ...
                {
                    Raison = string.Format(AkiledEnvironment.GetLanguageManager().TryGetValue("ban.1", Session.Langue));
                ...
                AkiledEnvironment.GetGame().GetClientManager().StaffAlert(RoomNotificationComposer.SendBubble("baneo", string.Format(AkiledEnvironment.GetLanguageManager().TryGetValue("BanAlertMessage.1", Session.Langue), Username), ""));
                Session.SendWhisper(string.Format(AkiledEnvironment.GetLanguageManager().TryGetValue("BanIPConfirmationMessage.1", Session.Langue), Username, Raison));
