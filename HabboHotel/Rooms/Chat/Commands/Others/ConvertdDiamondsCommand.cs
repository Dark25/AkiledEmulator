    ...
    class ConvertdDiamondsCommand : IChatCommand
    {
        public void Execute(GameClient Session, Room Room, RoomUser UserRoom, string[] Params)
        {
            ...
            try
            {
                ...
                if (TotalValue > 0)
                    Session.SendNotification("Todos sus diamantes en inventario se llevaron a su monedero con un !\r\r(Total de: " + TotalValue + " diamantes!");
                else
                    Session.SendNotification("Al parecer no tiene ningun otro articulo intercambiable!");
            }
            catch
            {
                Session.SendNotification("Oops, ocurrio un error mientras se intercambiaban sus diamantes, contacte un administrador!");
            }
        }
    }
}
