...
namespace Akiled.HabboHotel.Rooms.Wired.WiredHandlers.Conditions
{
    public class FurniStatePosMatch : IWiredCondition, IWired
    {
        ...
        public void SaveToDatabase(IQueryAdapter dbClient)
        {
            ...
            dbClient.SetQuery("DELETE FROM wired_items WHERE trigger_id = " + this.itemID + ";INSERT INTO wired_items (trigger_id,trigger_data,trigger_data_2,all_user_triggerable,triggers_item) VALUES (@id,@trigger_data,@trigger_data_2,@triggerable,@triggers_item);");
    ...
    public class ItemsPosReset
    {
        ...
        public ItemsPosReset(Item Item, int X, int Y, double Z, int Rot, string Extradata)
        {
            ...
            int result;
            if (int.TryParse(Extradata, out result) || (Extradata.Length < 10 && !Extradata.Contains(";") && !Extradata.Contains(":")))
            {
