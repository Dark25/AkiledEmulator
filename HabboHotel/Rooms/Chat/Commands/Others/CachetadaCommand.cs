...
namespace Akiled.HabboHotel.Rooms.Chat.Commands.Cmd
{
    class CachetadaCommand : IChatCommand
    {
        public void Execute(GameClient Session, Room Room, RoomUser UserRoom, string[] Params)
        {
            ...
            if (Params.Length == 1)
            {
                Session.SendWhisper("Introduce el nombre del usuario que deseas dar una cachetada(Bofetada).");
                return;
            }
            ...
            {
                Session.SendWhisper("Oops, al parecer el dueño de la sala ha prohibido los cachetadas (Bofetadas) en su sala.");
                return;
            }
            ...
            if (Session.GetHabbo().Rank <= 3)
            {
                if (!((Math.Abs(TargetUser.X - ThisUser.X) >= 2) || (Math.Abs(TargetUser.Y - ThisUser.Y) >= 2)))
                {
                    ...
                    Room.SendPacket(new ChatComposer(ThisUser.VirtualId, "@red@ * Dar Cachetada a " + Params[1] + " *", 0, ThisUser.LastBubble));
                    System.Threading.Thread.Sleep(500);
                    Room.SendPacketWeb(new PlaySoundComposer("cachetada", 2)); //Type = Trax
                    Room.SendPacket(new ChatComposer(TargetUser.VirtualId, "@red@ * Me han marcado la mano, en el cachete *" + Session.GetHabbo().Username + " :( *", 0, ThisUser.LastBubble));
                    System.Threading.Thread.Sleep(500);
