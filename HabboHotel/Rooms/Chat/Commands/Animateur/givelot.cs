    ...
    class givelot : IChatCommand
    {
        public void Execute(GameClient Session, Room Room, RoomUser UserRoom, string[] Params)
        {
            ...
            if (roomUserByHabbo.GetUsername() == Session.GetHabbo().Username || roomUserByHabbo.GetClient().GetHabbo().IP == Session.GetHabbo().IP)
            {
                ...
                AkiledEnvironment.GetGame().GetModerationManager().LogStaffEntry(Session.GetHabbo().Id, Session.GetHabbo().Username, 0, string.Empty, "notallowed", string.Format(AkiledEnvironment.GetLanguageManager().TryGetValue("givelot.1", Session.Langue), roomUserByHabbo.GetUsername()));
            ...
            UserRoom.SendWhisperChat(string.Format(AkiledEnvironment.GetLanguageManager().TryGetValue("givelot.2", roomUserByHabbo.GetClient().Langue), roomUserByHabbo.GetUsername(), NbLot, NbBadge));
