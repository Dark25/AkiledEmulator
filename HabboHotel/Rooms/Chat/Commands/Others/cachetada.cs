...
namespace Akiled.HabboHotel.Rooms.Chat.Commands.SpecialPvP
{
    class cachetada : IChatCommand
    {
        public void Execute(GameClient Session, Room Room, RoomUser UserRoom, string[] Params)
        {
            ...
            if (Params.Length == 1)
            {
                Session.SendWhisper("Por favor escriba el nombre del usuari@ que quieres cachetear.");
                return;
            }
            ...
            {
                Session.SendWhisper("Te quieres cachetear tu mism@? lol! ¿Estas bien?");
                return;
            }
            ...
            {
                Session.SendWhisper("No se puede cachetear a este usuario.");
                return;
            }
            ...
            if (!((Math.Abs(TargetUser.X - ThisUser.X) >= 2) || (Math.Abs(TargetUser.Y - ThisUser.Y) >= 2)))
            {
                ...
                Room.SendPacket(new ChatComposer(ThisUser.VirtualId, "@red@ *" + Params[1] + " ha recibido una cachetada en la cara*", 0, ThisUser.LastBubble));
                Room.SendPacket(new ChatComposer(TargetUser.VirtualId, "@cyan@ ¡Dios que cachetada! ¿Que fue lo que hice?", 0, ThisUser.LastBubble));
